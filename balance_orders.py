...
def merge_with_current_positions(df):
    ...
    current_pos_query = "SELECT code, pos, volume FROM mos.united_pos"
...
def merge_with_futures_details(df):
    ...
    from
    (SELECT code, bid/collateral as leverage, collateral, (bid+ask)/2 as price from mos.futquotes) fq
    inner join 
    (SELECT  * FROM mos.analytics_future) af
    on fq.code = af.sec
    left join
